...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 echo json_encode(["status" => "error", "error" => "Faltan
campos obligatorios"]);
 exit;
 }
...
ID: " . $pdo->lastInsertId()
 ...
 echo json_encode(["status" => "error", "error" => "El
correo ya está registrado"]);
 } else {
 http_response_code(500); // Error interno
 echo json_encode(["status" => "error", "error" => "Error en
BDD: " . $e->getMessage()]);
 }
 }
} else {
 ...
 echo json_encode(["status" => "error", "error" => "Método no
permitido"]);
}
?>